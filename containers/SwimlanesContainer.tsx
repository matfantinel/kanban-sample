...
const SwimlanesContainer: React.FC = () => {
  ...
  const handleTaskCreateClick = (status: TaskStatus) => {
    ...
    const newTask = {
      ...
      description: 'Task created via JS prompt',
      status,
    };
  ...
  const handleTaskEditClick = (task: Task) => {
    task.title = prompt('Enter task title', task.title) ?? 'Edited task';
    updateTask(task);
  };
  ...
  return (
    <Swimlanes>
      <DragDropContext onDragEnd={onDragEnd}>
        <Swimlane title='To-Do' status={TaskStatus.ToDo} onCreateClick={() => handleTaskCreateClick(TaskStatus.ToDo)}>
        ...
        <Swimlane
          ...
          onCreateClick={() => handleTaskCreateClick(TaskStatus.InProgress)}
        >
          {buildSwimlaneContent(TaskStatus.InProgress)}
        </Swimlane>

        <Swimlane title='Done' status={TaskStatus.Done} onCreateClick={() => handleTaskCreateClick(TaskStatus.Done)}>
